...
export default function AdminMatchUpdate() {
  ...
  // Toggle modal
  const hiddenShow = () => setShowStats((prev) => !prev);
  ...
  // === FETCH DATA ===
  // 1. Lấy thông tin trận đấu + ticketSettings
  useEffect(() => {
    const fetchMatchDetail = async () => {
      ...
      try {
        ...
        if (response.data?.status === "success") {
          ...
          setPreviewUrls({
            homeLogo: matchData.homeLogoUrl || null,
            awayLogo: matchData.awayLogoUrl || null,
            banner: matchData.matchImageUrl || null,
          });

          // Xử lý ticketSettings
          const settingsMap = {};
          if (matchData.ticketSettings && matchData.ticketSettings.length > 0) {
            matchData.ticketSettings.forEach((setting) => {
              settingsMap[setting.sectionId] = {
                quantity: setting.totalQuantity,
                price: setting.price,
              };
            });
          }
  ...
  useEffect(() => {
    const fetchTournaments = async () => {
      try {
        ...
        console.error("Lỗi khi gọi API tournaments:", error);
      } finally {
  ...
  // 3. Lấy danh sách sections (từ API chung - không phụ thuộc matchId)
  useEffect(() => {
    const fetchSections = async () => {
      ...
      try {
        ...
        if (response.data?.status === "success") {
          setSections(response.data.data); // [{ id: 1, name: "CE1" }, ...]
        } else {
  ...
  const handleSectionClick = (sectionId) => {
    ...
    const existing = ticketSettings[sectionId];
    setTempQuantity(existing?.quantity?.toString() || "");
    setTempPrice(existing?.price?.toString() || "");
    setShowStatsIn(true);
  };

  const handleSaveTicketSetting = () => {
    if (!selectedSection || tempQuantity === "" || tempPrice === "") return;
  ...
  const handleSubmit = async (e) => {
    ...
    const dataPayload = {
      ...
      ticketSettings: sections.map((section) => {
        const userSetting = ticketSettings[section.id];
        return {
          sectionId: section.id,
          totalQuantity: userSetting?.quantity ?? 500,
          price: userSetting?.price ?? 100.0,
        };
      }),
    };

    // === 2. DÙNG BLOB ĐỂ GỬI data (Spring Boot yêu cầu part dạng text) ===
    const dataBlob = new Blob([JSON.stringify(dataPayload)], {
      type: "application/json",
    });
    formDataToSend.append("data", dataBlob);

    // === 3. GỬI FILE RIÊNG (chỉ khi người dùng chọn file mới) ===
    ...
    try {
      const response = await api.put(
        `api/matches-and-tickets/update/match/${matchId}`,
        formDataToSend,
        {
          headers: {
            // Không set Content-Type → để browser tự set boundary
            // "Content-Type": "multipart/form-data" → SAI, sẽ lỗi
          },
        }
      );
  ...
  // === RENDER ===
  if (loading || loadingSections || loadingTournaments) {
    return (
          ...
          >
            ...
            <label className="flex flex-col w-full relative">
              ...
              <select
                ...
                onChange={(e) =>
                  handleInputChange("tournamentId", e.target.value)
                }
            ...
            <div className="flex gap-5 w-full justify-between">
              <label className="flex flex-col w-[48%] relative">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("matchDate", e.target.value)
                  }
                  className={`border rounded-[10px] h-10 px-3 mt-2 ${
