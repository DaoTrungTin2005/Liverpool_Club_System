...
public class PlayerController {
    ...
    public ResponseEntity<ApiResponse<PlayerDetailWithStatsResponseDTO>> updatePlayer(
            ...
            @RequestPart("player") @Valid CreatePlayerRequest dto, // CREATE HAY UPDATE CŨNG nhận format i chang nên xài lun
            @RequestPart(value = "bioImage", required = false) MultipartFile bioImage,
            @RequestPart(value = "backgroundImage", required = false) MultipartFile backgroundImage) {
    ...
    @GetMapping("/list")
    public ResponseEntity<ApiResponse<List<PlayerDetailWithStatsResponseDTO>>> getAllPlayers() {
        List<PlayerDetailWithStatsResponseDTO> players = playerService.getAllPlayersWithStats();
